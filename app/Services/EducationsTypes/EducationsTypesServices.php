...
use App\Repositories\EducationsTypesRepository;
use Carbon\Carbon;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Log;

class EducationsTypesServices implements EducationsTypesInterface
{
    
    protected $type_repository;
    public function __construct(EducationsTypesRepository $type_repository)
    {
        $this->type_repository = $type_repository;   
    }
    public function index($params) {
        try {
            $model = $this->type_repository;
    ...
    public function details($id) {
       try {
         $model = $this->type_repository->where('id', $id, '=')->first();
         if(isset($model->id)) {
            $result = [
                "code" => 200,                
                "data" => $model
    ...
    public function create($params) {
       try {
            ...
            $model = $this->type_repository->create($data);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function createMultiple($params) {
        try {
            foreach ($params['name'] as $name) {
             ...
             $model = $this->type_repository->createMultiple($data);             
             $result = null;
             if(count($model) > 0) {
                 $result = [
                     "code" => 200,
    ...
    public function update($params, $id) {
        ...
        try {
            ...
            $model = $this->type_repository->updateById($id, $params);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function delete($id) {
        try {
            ...
            $model = $this->type_repository->updateById($id, $data); 
            $result = null;
            if($model) {
                $result = [
                    "code" => 200,
