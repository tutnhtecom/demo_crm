...
class StudentsExports implements FromCollection, WithHeadings, WithMapping
{
    protected $data;
    ...
    public function map($item):array{        
        ...
        if(isset($item->contacts)) {
            $contacts = $item->contacts->where('students_id', $item->id)->where('type', 0)->first();            
        }
        ...
        $data = [
            $i,
            $item->students_code ?? null,
            $item->full_name ?? null,
            $date_of_birth,
            $item->phone ?? null,
            $item->email ?? null,
            $address ?? null,
            $item->employees->name ?? 'Chưa có nhân viên tư vấn',
            $status,
            $sources_name ?? "",
