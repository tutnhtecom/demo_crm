...
class LeadsExports implements  FromCollection, WithMapping, WithHeadings
{
    protected $data;    
    protected $fields;    
    protected $rm_fields;    
    ...
    public function map($item):array{                          
        ...
        foreach ($this->fields as $field) {
            ...
            if(isset($field["field_name"]) &&  $field["field_name"] ==  "contacts_hktt") {
                $contacts = isset($item->contacts) ? $item->contacts->where('leads_id', $item->id)->where('type', 1)->first() : null;
