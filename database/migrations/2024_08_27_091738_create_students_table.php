...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('students', function (Blueprint $table) {
            ...
            $table->string('code')->unique()->nullable(TRUE)->default(NULL)->comment('Lưu mã của thí sinh, sinh viên');
            ...
            //Khóa quan hệ
            $table->bigInteger('leads_id')->unsigned()->nullable()->default(NULL)->comment('Lưu id của dân tộc, là khóa ngoại quan hệ với bảng ethnics');
            ...
            // $table->string('nations_id')->unsigned()->nullable()->default(NULL)->comment('Lưu id của quốc gia, là khóa ngoại quan hệ với bảng nations');
            // $table->bigInteger('ethnics_id')->unsigned()->nullable()->default(NULL)->comment('Lưu id của dân tộc, là khóa ngoại quan hệ với bảng ethnics');
            $table->string('nations_name')->nullable()->default(NULL)->comment('Lưu id của quốc gia, là khóa ngoại quan hệ với bảng nations');
            $table->string('ethnics_name')->nullable()->default(NULL)->comment('Lưu id của dân tộc, là khóa ngoại quan hệ với bảng ethnics');
            ...
            $table->foreign('lst_status_id')->references('id')->on('lst_status')->onDelete('cascade'); // trạng thái thí sinh và sinh viên        
            $table->foreign('tags_id')->references('id')->on('tags')->onDelete('cascade'); // thẻ 

            // đánh index
            $table->index('email'); 
            $table->index('phone'); 
            $table->index('identification_card');
