...
@section('content')
        ...
        <div class="app_breadcrumb d-flex align-items-center justify-content-between">
            ...
            <div class="x-3 py-4">
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-primary">Nguồn tiếp cận</li>
                    <!--end::Item-->
                </ul>
            </div>
            <!--end:Breadcrumb-->
                ...
                <div
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <div class="d-flex md:d-block align-items-center gy-2 gap-1">
                            <a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#ti_modal_add_sources"
                    ...
                    <table class="table table-sm table-striped table-crm table-row-devider-300 bordered rounded-3 m-0 w-100"
                        ...
                        <thead>
                            <tr class="bg-primary text-white">
                                <th class="text-nowrap align-middle fs-5 text-center px-4" style="max-width:50px;">ID</th>
                                <th class="text-nowrap align-middle fs-5 text-start px-4">Tên Nguồn tiếp cận</th>
                        ...
                        <tbody>
                            @if(!isset($data['message']))
                                @foreach ($data['data'] as $status)
                                    <tr>
                                        <td class="align-middle px-2 px-md-4 py-4 text-primary text-center"> {{$status['id']}} </td>
                                        <td class="align-middle px-2 px-md-4 py-4 text-primary"> {{$status['name']}} </td>

                                        <td class="align-middle px-2 px-md-4 py-4 text-center">
                                            {{ $status['created_at'] ? \Carbon\Carbon::parse($status['created_at'])->timezone('Asia/Ho_Chi_Minh')->format('d/m/Y') : '-'  }}                                            
                                        </td>
                                        <td class="align-middle text-center">
                                                <button type="button" class="btn btn-ghost p-1"
                                                    data-bs-toggle="modal" data-bs-target="#editStatusModal{{$status['id']}}" data-name-status="{{ $status['name'] }}"">
                                                    <img src="assets/crm/media/svg/crm/edit.svg" alt="Sửa" width="18"
                                                        height="18" />
                                                </button>
                                                @include('crm.content.systemConfig.modal_edit_sources')

                                                <button type="button" class="btn btn-ghost p-1"
                                                    data-ti-row-confirm-message="Xóa bản ghi này?" data-ti-button-action="row-remove"
                                                ...
                                                @include('crm.content.systemConfig.modal_delete_sources')
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
    ...
    @include('crm.content.systemConfig.modal_add_sources')
    ...
    <script type="module" src="/assets/crm/js/htecomJs/addNewSources.js"></script>
    <script type="module" src="/assets/crm/js/htecomJs/editSources.js"></script>
    <script type="module" src="/assets/crm/js/htecomJs/deleteSources.js"></script>
@endsection
