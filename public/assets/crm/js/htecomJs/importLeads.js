...
import { importLeadsApi } from '/assets/crm/js/htecomJs/variableApi.js';

$(document).ready(()=>{

    $('#import_leads_file').change(function() {
        $('.error_log_leads').empty();
        $('.error_log_leads').hide();
    });

    $('#import_btn_leads').click(() => {        
        const button = $('#import_btn_leads');
        button.addClass('disable');
        button.html('Đang tải...');
        const api = baseUrl + importLeadsApi;
        const token = localStorage.getItem('jwt_token');
        const formData = new FormData();
        let file = $('#import_leads_file')[0].files[0];        
        ...
        if (!allowedMimeTypes.includes(mimeType)) {
            ...
            $('#import_leads_file').val(''); // Xóa file không hợp lệ
            button.html('Tải lên');
            button.removeClass('disable');
            return;
        }
            ...
            success: (res) => {
                if (res.code == 422) {
                    $.each(res.message, function(index, item) {
                        ...
                        `;
                        $('.error_log_leads.leads_err').append(errorHTML);
                    });
                    $('.error_log_leads').show();
                    $('#import_leads_file').val('');
                } else {
                    $('#import_leads_file').val('');
