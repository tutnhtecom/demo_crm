...
import { importSourcesApi } from '/assets/crm/js/htecomJs/variableApi.js';

$(document).ready(()=>{

    $('#import_sources_file').change(function() {
        $('.error_log_leads').empty();
        $('.error_log_leads').hide();
    });
    // $('#import_btn_sources').click(()=>{        
    //     const button = $('#import_btn_sources');
    //     button.addClass('disable');
    //     button.html('Đang tải...');
    //     const api       = baseUrl+importSourcesApi;
    //     const token     = localStorage.getItem('jwt_token');
    //     const formData  = new FormData();
    //     let file        = $('#import_sources_file')[0].files[0];        
    //     if (!file) {
    ...
    $('#import_btn_sources').click(() => {        
        ...
        if (!allowedMimeTypes.includes(mimeType)) {
            ...
            $('#import_sources_file').val(''); // Xóa file không hợp lệ
            button.html('Tải lên');
            button.removeClass('disable');
            return;
        }
