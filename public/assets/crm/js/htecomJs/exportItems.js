...
import { exportLeadsApi, exportStudentsApi } from '/assets/crm/js/htecomJs/variableApi.js';
...
$(document).ready(()=>{
    ...
    $(document).on('click', '#btn_export_items', function(){       
        let itemType = $(this).attr('data-type');
        let apiURL ='';
        if(itemType =='lead'){
            apiURL = exportLeadsApi;
        }else{
            apiURL = exportStudentsApi;
        }
        const token = localStorage.getItem('jwt_token');
        let keywords = $('#search-table-leads').val();
        var d = new Date();
        var month = d.getMonth()+1;
        var day = d.getDate();
        var file_name = 'danh_sach_sinh_vien_tiem_nang_' + d.getFullYear() +
        (month<10 ? '0' : '') + month +
        (day<10 ? '0' : '') + day;
    ...
    $(document).on('change', 'input[name="check-field"]', function(){ 
        ...
        if (!this.checked) {
            // Nếu checkbox bị bỏ chọn, thêm giá trị vào mảng nếu chưa có
            if (!rm_fields.includes(fieldValue)) {
                rm_fields.push(fieldValue);
            }
        } else {
            // Nếu checkbox được chọn lại, xóa giá trị khỏi mảng
    ...
    $(document).on('click', '#unSelectAll', (e)=>{
        const idForm = $(e.currentTarget).attr('data-id-form');
